...
const SelectionForm: FC = () => {
  ...
  let sumDuration = 0;
  selected.services.forEach(
    (service: IService) => (sumDuration += service.duration)
  );
  ...
  const autoSelectFirstDoctor = () => {
    ...
    if (dataState.employeesList[0].uid != "") {
      ...
      if (
        appState.selected.employee.uid == "" &&
        appState.selected.employee.name == ""
      ) {
        appState.selected.employee.uid =
          dataState.employeesList[randomEmployee].uid; //Работает но с предупреждением MobX
        appState.selected.employee.name =
          dataState.employeesList[randomEmployee].name;
      }
    } else {
    }
  };
